...
IUSE="compat-ethread doc emacs halfword hipe java kpoll odbc smp sctp ssl systemd tk wxwidgets valgrind debug"
...
pkg_setup() {
	...
	if use valgrind ; then
		use !hipe || die "valgrind conflict with hipe"
	fi
}
...
src_compile() {
	...
	if use debug ; then
		einfo "debug support compile"
		pushd erts/emulator
		export TARGET=$CHOST
		export ERL_TOP=${S}
		emake debug FLAVOR=
		emake debug FLAVOR=smp
		unset ERL_TOP
		popd
	fi

	if use valgrind ; then
		einfo "valgrind support compile"
		pushd erts/emulator
		export TARGET=$CHOST
		export ERL_TOP=${S}
		emake valgrind FLAVOR=
		emake valgrind FLAVOR=smp
		unset ERL_TOP
		popd
	fi
}
...
src_install() {
	...
	einfo "sed ${S}/bin/cerl"
	sed -e "s:^ROOTDIR=.*:ROOTDIR=${ERL_LIBDIR}:" -e "s:^BINDIR=.*:BINDIR=${ERL_LIBDIR}/erts-${ERL_ERTS_VER}/bin:" -i "${S}/bin/cerl"
	#remove magic -pz
	sed -e 's:-pz $PRELOADED::' -i "${S}/bin/cerl"
	if use debug || valgrind ; then
		cp ${S}/bin/cerl ${D}${ERL_LIBDIR}/bin
	fi	
	dosym ${ERL_LIBDIR}/bin/cerl /usr/bin/cerl

	if use debug ; then
		cp ${S}/bin/${CHOST}/beam.debug ${D}${ERL_LIBDIR}/erts-${ERL_ERTS_VER}/bin/
		cp ${S}/bin/${CHOST}/beam.debug.smp ${D}${ERL_LIBDIR}/erts-${ERL_ERTS_VER}/bin/
	fi

	if use valgrind ; then
		cp ${S}/bin/${CHOST}/beam.valgrind ${D}${ERL_LIBDIR}/erts-${ERL_ERTS_VER}/bin/
		cp ${S}/bin/${CHOST}/beam.valgrind.smp ${D}${ERL_LIBDIR}/erts-${ERL_ERTS_VER}/bin/
	fi
